...
> ⚠️ **본 문서는 설계 원안입니다.** 핵심 개념(4노드 파이프라인, 쌍방 Supervisor 진화 루프)은 그대로 구현되었지만,
> 세부 구현은 다릅니다 — 예: Environment Provisioner는 Docker/uv 패키징 대신 **subprocess 기반 도구 런타임**(`services/mcp/tool_runtime.py`)으로,
> 샌드박스 테스트는 별도 격리 환경 대신 **dry-run 실행**으로 구현되었습니다.
> **실제 동작·API·환경 변수는 [`구현_현황.md`](./구현_현황.md)를 우선 참고하세요.**
