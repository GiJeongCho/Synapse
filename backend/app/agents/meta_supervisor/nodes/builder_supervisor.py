...
def _runtime_test_from_completion(project_files: dict[str, Any]) -> dict[str, Any]:
    """Completer가 수행한 dry-run 런타임 검사 결과를 runtime_test 형태로 변환한다.

    중복 dry-run을 피하기 위해 Builder는 직접 smoke test를 돌리지 않고
    파이프라인(Completer)이 남긴 completion_report.runtime 을 재사용한다.
    """
    report = project_files.get("completion_report", {}) if isinstance(project_files, dict) else {}
    health = report.get("runtime", {}) if isinstance(report, dict) else {}
    if not isinstance(health, dict) or not health.get("ran"):
        return {"ran": False, "failed": False, "failure_reason": "", "health": {}}

    issues = health.get("issues", []) if isinstance(health.get("issues"), list) else []
    failed = not health.get("healthy", True)
    return {
        "ran": True,
        "failed": failed,
        "failure_reason": "; ".join(issues) if failed else "",
        "health": health,
    }
...
async def builder_supervisor(
    ...
    strategy = await ainvoke_json(
        llm, messages, node="builder", retries=1,
        fallback={"confidence": 0.5, "approach": "default"},
    )
    ...
    # Completer가 이미 파이프라인 안에서 dry-run 런타임 검사를 수행했으므로
    # 그 결과를 재사용한다 (중복 dry-run 제거 — 라운드당 검사 1회).
    runtime_test = _runtime_test_from_completion(project_files)
    ...
    current_result = {
        ...
        "plan": project_files.get("plan", {}),
        "completion_report": project_files.get("completion_report", {}),
    }
    ...
    return {
        ...
        "history": [{
            ...
            "runtime_failed": runtime_test.get("failed", False),
            "runtime_reason": runtime_test.get("failure_reason", ""),
        }],
    }
